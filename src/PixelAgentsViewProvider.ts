...
import type { AgentRoster } from './rosterPersistence.js';
import { readRoster, writeRoster } from './rosterPersistence.js';
...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
  ...
  private initHooks(): void {
    ...
    this.pixelAgentsServer.onSendMessage((message) => {
      for (const agent of this.agents.values()) {
        if (!agent.terminalRef || !agent.isWaiting) continue;
        // \x15 clears any partial readline input before injecting the message.
        // Claude Code runs in raw TTY mode so \r triggers Enter, not \n.
        agent.terminalRef.sendText('\x15' + message + '\r', false);
      }
    });
    ...
    webviewView.webview.onDidReceiveMessage(async (message) => {
      if (message.type === 'openClaude') {
        ...
        await launchNewTerminal(
          ...
          message.isCeo as boolean | undefined,
        );
      ...
      } else if (message.type === 'resetAgentCounter') {
        // Reset ID counters so the next spawned agent starts at 1
        this.nextAgentId.current = 1;
        this.nextTerminalIndex.current = 1;
        await this.context.workspaceState.update(WORKSPACE_KEY_AGENTS, []);
        await this.context.workspaceState.update(WORKSPACE_KEY_AGENT_SEATS, {});
      } else if (message.type === 'sendAgentMessage') {
        const id = message.id as number;
        const text = (message.message as string ?? '').trim();
        const agent = this.agents.get(id);
        if (agent?.terminalRef && text) {
          // Append \r explicitly — Claude Code runs in raw TTY mode where \r triggers Enter.
          // VS Code's sendText(text, true) appends \n on Mac/Linux which does not execute.
          agent.terminalRef.sendText(text + '\r', false);
        }
      } else if (message.type === 'approvePermission') {
        const id = message.id as number;
        const agent = this.agents.get(id);
        if (agent?.terminalRef) {
          agent.terminalRef.sendText('\r', false);
        }
        ...
        if (uris && uris[0]) {
          this.webview?.postMessage({
            type: 'folderSelected',
            agentId: message.agentId as number,
            path: uris[0].fsPath,
          });
        }
      } else if (message.type === 'saveAgentRoster') {
        const roster = message.roster as AgentRoster;
        writeRoster(roster);
        vscode.window.showInformationMessage(`Pixel Agents: Roster saved (${roster.agents.length} agent${roster.agents.length === 1 ? '' : 's'}).`);
      } else if (message.type === 'loadAgentRoster') {
        const roster = readRoster();
        this.webview?.postMessage({ type: 'agentRosterLoaded', roster });
      } else if (message.type === 'exportAgentRoster') {
        const roster = readRoster();
        if (!roster) {
          vscode.window.showWarningMessage('Pixel Agents: No saved roster to export.');
          return;
        }
        const uri = await vscode.window.showSaveDialog({
          filters: { 'JSON Files': ['json'] },
          defaultUri: vscode.Uri.file(path.join(os.homedir(), 'pixel-agents-roster.json')),
        });
        if (uri) {
          fs.writeFileSync(uri.fsPath, JSON.stringify(roster, null, 2), 'utf-8');
          vscode.window.showInformationMessage('Pixel Agents: Roster exported successfully.');
        }
      } else if (message.type === 'importAgentRoster') {
        const uris = await vscode.window.showOpenDialog({
          filters: { 'JSON Files': ['json'] },
          canSelectMany: false,
        });
        ...
        try {
          const raw = fs.readFileSync(uris[0].fsPath, 'utf-8');
          const imported = JSON.parse(raw) as AgentRoster;
          if (imported.version !== 1 || !Array.isArray(imported.agents)) {
            vscode.window.showErrorMessage('Pixel Agents: Invalid roster file.');
            return;
          }
          writeRoster(imported);
          this.webview?.postMessage({ type: 'agentRosterLoaded', roster: imported });
          vscode.window.showInformationMessage('Pixel Agents: Roster imported successfully.');
        } catch {
          vscode.window.showErrorMessage('Pixel Agents: Failed to read or parse roster file.');
        }
      }
    });
