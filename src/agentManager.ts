...
// Common install locations for Claude Code CLI, checked at startup
...
export async function launchNewTerminal(
  ...
  // Ensure ~/.local/bin (Claude Code's default install location) is in PATH
  const localBin = path.join(os.homedir(), '.local', 'bin');
  const currentPath = process.env.PATH ?? '';
  const envPath = currentPath.includes(localBin) ? currentPath : `${localBin}:${currentPath}`;
  ...
  // Quote the binary path so spaces work; if CLAUDE_BIN is the bare fallback,
  // prepend PATH so the shell can find it even without a login profile.
  const binInvocation = CLAUDE_BIN === 'claude'
    ? `PATH="$HOME/.local/bin:/usr/local/bin:$PATH" claude`
    : `"${CLAUDE_BIN}"`;
  const claudeCmd = bypassPermissions
    ? `${binInvocation} --session-id ${sessionId} --dangerously-skip-permissions`
    : `${binInvocation} --session-id ${sessionId}`;
  ...
  webview?.postMessage({ type: 'agentCreated', id, folderName, isCeo: agent.isCeo });
...
export function sendExistingAgents(
  ...
  // Include folderName, isExternal, and isCeo per agent
