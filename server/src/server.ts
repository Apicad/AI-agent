...
/** Callback invoked when a broadcast message is received via POST /api/send-message. */
 ...
 * - `POST /api/send-message` -- broadcast message to all terminals (auth required)
...
export class PixelAgentsServer {
  ...
  /** Register a callback for broadcast messages sent to all terminals. */
  onSendMessage(callback: SendMessageCallback): void {
  ...
  private handleRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
    ...
    // Broadcast message endpoint: POST /api/send-message
    if (req.method === 'POST' && url === '/api/send-message') {
  ...
  /** Handle POST /api/send-message. Validates auth, reads message, broadcasts to all terminals. */
  private handleSendMessageRequest(req: http.IncomingMessage, res: http.ServerResponse): void {
    const authHeader = req.headers['authorization'] ?? '';
    const expectedToken = `Bearer ${this.config?.token ?? ''}`;
    const authBuf = Buffer.from(authHeader);
    const expectedBuf = Buffer.from(expectedToken);
