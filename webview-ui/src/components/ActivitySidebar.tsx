...
import type { AgentHistoryEntry, ChatMessage } from '../hooks/useExtensionMessages.js';
...
interface ActivitySidebarProps {
  ...
  onSetAgentMeta: (id: number, updates: { name?: string; task?: string; folderPath?: string; mode?: string; homeZoneId?: string; role?: string }) => void;
...
function AgentCard({
  ...
  onClose: () => void;
  onRelaunch?: () => void;
  onSetAgentMeta: (updates: { name?: string; task?: string; folderPath?: string; mode?: string; homeZoneId?: string; role?: string }) => void;
  ...
  const [barState, setBarState] = useState<{ visible: boolean; pct: number }>({ visible: false, pct: 0 });
  ...
  const [attachments, setAttachments] = useState<{ id: string; type: 'file' | 'image' | 'url'; label: string; value: string }[]>([]);
  ...
  const commitWithFlash = (updates: { name?: string; task?: string; folderPath?: string; mode?: string; homeZoneId?: string; role?: string }) => {
    onSetAgentMeta(updates);
    setSavedFlash(true);
    setTimeout(() => setSavedFlash(false), 1500);
  };

  useEffect(() => { setNameInput(customName ?? ''); }, [customName]);
  useEffect(() => { setTaskInput(task ?? ''); }, [task]);
  ...
  useEffect(() => {
    ...
    setAttachments((a) => [...a, { id: Date.now().toString(), type: isImg ? 'image' : 'file', label, value: pendingFileAttach }]);
    onClearPendingFileAttach?.();
  }, [pendingFileAttach]);
  ...
  const statusLabel = hasPermission ? 'needs approval' : isWaiting ? 'done' : isActive ? 'active' : 'idle';
    ...
    <div style={{
      ...
      {barState.visible && (
        <div style={{ height: 3, background: 'var(--progress-track-bg)', overflow: 'hidden' }}>
          <div style={{
            height: '100%', width: `${barState.pct}%`,
            background: barState.pct >= 100 ? 'var(--color-status-success)' : 'var(--color-status-active)',
      ...
      <div style={{ padding: '10px 12px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          {/* Status pill */}
          <span style={{
            fontSize: 16, padding: '3px 10px', borderRadius: 3, flexShrink: 0, fontWeight: 600,
          ...
          {isEditingName ? (
            <input
              ...
              style={{
                flex: 1, background: 'var(--color-bg-dark)', border: '1px solid var(--color-border)',
                color: 'var(--color-text)', fontSize: 26, fontWeight: 'bold',
                fontFamily: 'FS Pixel Sans, monospace', padding: '1px 6px', outline: 'none',
              }}
            />
          ) : (
            <span
              ...
              style={{
                flex: 1, fontWeight: 'bold', fontSize: 26,
          ...
          {isCeo && (
            <span style={{
              fontSize: 15, padding: '2px 9px', borderRadius: 3, flexShrink: 0,
              background: `${CEO_COLOR}22`, border: `1px solid ${CEO_COLOR}`, color: CEO_COLOR,
          ...
          {!isCeo && effectiveRole === 'manager' && (
            <span style={{
              fontSize: 15, padding: '2px 9px', borderRadius: 3, flexShrink: 0,
              background: `${MGR_COLOR}22`, border: `1px solid ${MGR_COLOR}`, color: MGR_COLOR,
              fontWeight: 700, letterSpacing: '0.06em',
            }}>◆ MGR</span>
          )}
          {!isCeo && effectiveRole === 'worker' && (
            <span style={{
              fontSize: 15, padding: '2px 9px', borderRadius: 3, flexShrink: 0,
              background: `${WRK_COLOR}18`, border: `1px solid ${WRK_COLOR}55`, color: WRK_COLOR,
              fontWeight: 700, letterSpacing: '0.06em',
            }}>· WRK</span>
          )}

          {/* Dice + flash + close */}
          <button onClick={pickRandomName} title="Random name" style={{
            background: 'none', border: 'none', color: 'var(--color-text-muted)',
            cursor: 'pointer', fontSize: 17, padding: '0 2px', flexShrink: 0, opacity: 0.6,
          }}>🎲</button>
          {savedFlash && (
            <span style={{ fontSize: 16, color: 'var(--color-status-success)', flexShrink: 0 }}>✓</span>
          )}
          {onRelaunch && (
            <button onClick={onRelaunch} title="Relaunch CEO" style={{
              background: `${CEO_COLOR}22`, border: `1px solid ${CEO_COLOR}`, color: CEO_COLOR,
              cursor: 'pointer', fontSize: 14, padding: '2px 8px', flexShrink: 0,
              fontFamily: 'FS Pixel Sans, monospace',
            }}>⟳</button>
          )}
          <button onClick={onClose} title="Close agent" style={{
            background: 'none', border: 'none', color: 'var(--color-text-muted)',
            cursor: 'pointer', fontSize: 20, padding: '0 2px', flexShrink: 0,
          }}>✕</button>
        </div>

        {/* ── Meta row: effort · folder ── */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, fontSize: 17 }}>
          {effortIdx > 0 && (
            <span style={{
              ...
              flexShrink: 0,
            }}>⚡ {EFFORT_LEVELS[effortIdx].label}</span>
          )}
          <span
            title={folderPath ?? folderName ?? ''}
            style={{
              flex: 1, color: 'var(--color-text-muted)', overflow: 'hidden', textOverflow: 'ellipsis',
              whiteSpace: 'nowrap', opacity: 0.7, fontSize: 15,
            }}
          >
            📁 {folderPath
        ...
        {hasPermission && (
          <div style={{
            ...
            padding: '8px 12px', marginBottom: 10, borderRadius: 2,
          }}>
            <span style={{ fontSize: 19, color: 'var(--color-status-permission)', fontWeight: 600 }}>
              ⚠ Waiting for approval
            </span>
            <button
              onClick={() => vscode.postMessage({ type: 'approvePermission', id: agentId })}
              style={{
                background: 'var(--color-status-permission)', border: 'none',
                color: 'var(--color-bg)', cursor: 'pointer', fontSize: 17,
                fontWeight: 'bold', fontFamily: 'FS Pixel Sans, monospace',
        ...
        <div style={{ marginBottom: 10, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') { commitWithFlash({ task: taskInput }); (e.target as HTMLInputElement).blur(); }
              if (e.key === 'Escape') { setTaskInput(task ?? ''); (e.target as HTMLInputElement).blur(); }
            }}
            ...
            style={{
              ...
              fontSize: 20, fontFamily: 'FS Pixel Sans, monospace',
          ...
          <button
            ...
            style={{
              ...
              fontSize: 17, fontFamily: 'FS Pixel Sans, monospace',
              padding: '3px 10px', flexShrink: 0,
              opacity: taskInput.trim() ? 1 : 0.4, borderRadius: 2,
            }}
          >▶</button>
        </div>
        ...
        {taskSentNotice && (
          <div style={{
            ...
            padding: '5px 10px', marginBottom: 8, fontSize: 16,
            color: 'var(--color-status-active)', borderRadius: 2,
          }}>
            <span>▶</span><span>Task sent</span>
          </div>
        )}
        ...
        <div style={{ marginBottom: 10, display: 'flex', gap: 5 }}>
          ...
          {(() => {
            ...
            return (
              <button
                ...
                style={{
                  flex: 1, background: isPlan ? 'var(--color-status-active)22' : 'transparent',
                  ...
                  cursor: 'pointer', fontSize: 18, padding: '7px 0',
                  fontFamily: 'FS Pixel Sans, monospace',
                }}
              >{isPlan ? '🔵 Plan' : 'Plan'}</button>
            );
          })()}
            ...
            return (
              <button
                onClick={() => {
                  ...
                  setEffortIdx(next);
                  if (currentMode !== 'planner') commitWithFlash({ mode: EFFORT_LEVELS[next].mode });
                }}
                title="Cycle effort: Low → High → Max"
                style={{
                  flex: 1, background: effortIdx > 0 ? `${effort.color}22` : 'transparent',
          ...
          {!isCeo && (
            <button
              onClick={() => commitWithFlash({ role: effectiveRole === 'manager' ? 'worker' : 'manager' })}
              title="Toggle role"
              style={{
                flex: 0, background: effectiveRole === 'manager' ? `${MGR_COLOR}22` : 'transparent',
                ...
                cursor: 'pointer', fontSize: 18, padding: '7px 12px',
                fontFamily: 'FS Pixel Sans, monospace',
              }}
            >{effectiveRole === 'manager' ? '◆ MGR' : '· WRK'}</button>
          )}
        </div>
          ...
          <div style={{ marginBottom: 8 }}>
            {activeTools.map((tool) => (
              <div key={tool.toolId} style={{
                ...
                fontSize: 19, color: tool.permissionWait ? 'var(--color-status-permission)' : 'var(--color-text)',
                padding: '3px 0',
              }}>
                <span className="pixel-pulse" style={{ color: 'var(--color-status-active)', flexShrink: 0, fontSize: 16 }}>▶</span>
                <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', opacity: 0.85 }} title={tool.status}>
                  {tool.status}
                </span>
              </div>
            ))}
        ...
        {history.length > 0 && (
          <>
            <div style={{ borderTop: '1px solid var(--color-border)', marginBottom: 4, opacity: 0.3 }} />
            {history.slice(0, 6).map((entry) => {
              const totalTokens = (entry.inputTokens ?? 0) + (entry.outputTokens ?? 0);
              const isWaitingEntry = entry.type === 'waiting';
              return (
                <div key={entry.entryId} style={{ display: 'flex', flexDirection: 'column', gap: 1, padding: '3px 0' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 18, color: 'var(--color-text-muted)' }}>
                    <span style={{
                      flexShrink: 0, fontSize: 16,
                      color: isWaitingEntry ? 'var(--color-status-success)' : entry.type === 'permission' ? 'var(--color-status-permission)' : 'var(--color-text-muted)',
                    }}>
                      {isWaitingEntry ? '✓' : '·'}
                    </span>
                    <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={entry.statusText}>
                      {entry.statusText}
                    </span>
                    <span style={{ flexShrink: 0, fontSize: 16, opacity: 0.7 }}>
                      {entry.durationMs !== undefined ? formatDuration(entry.durationMs) : timeAgo(entry.timestamp, now)}
                    </span>
                  </div>
                  {isWaitingEntry && totalTokens > 0 && (
                    <div style={{ display: 'flex', gap: 12, fontSize: 16, paddingLeft: 22, color: 'var(--color-text-muted)', opacity: 0.7 }}>
                      {entry.inputTokens !== undefined && <span>↑ {formatTokens(entry.inputTokens)}</span>}
                      {entry.outputTokens !== undefined && <span>↓ {formatTokens(entry.outputTokens)}</span>}
        ...
        {activeTools.length === 0 && history.length === 0 && (
          <div style={{ fontSize: 18, color: 'var(--color-text-muted)', padding: '4px 0', opacity: 0.6 }}>No activity yet</div>
        )}
              ...
              {chatMessages.slice(-20).map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <span style={{
                    ...
                    background: m.role === 'user' ? 'var(--color-accent)18' : 'var(--color-bg-dark)',
                    border: `1px solid ${m.role === 'user' ? 'var(--color-accent)44' : 'var(--color-border)'}`,
                    padding: '5px 10px', maxWidth: '90%', wordBreak: 'break-word', whiteSpace: 'pre-wrap',
                    borderRadius: 2,
                  }}>
                    {m.text}
                  </span>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 6 }}>
              {attachments.map((a) => (
                <span key={a.id} style={{
                  ...
                  fontSize: 16, padding: '3px 8px', maxWidth: 200,
                  fontFamily: 'FS Pixel Sans, monospace', borderRadius: 2,
                }}>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && urlInput.trim()) {
                    setAttachments((a) => [...a, { id: Date.now().toString(), type: 'url', label: urlInput.trim(), value: urlInput.trim() }]);
                    setUrlInput(''); setShowUrlInput(false);
                  }
                  if (e.key === 'Escape') { setUrlInput(''); setShowUrlInput(false); }
                }}
                placeholder="Paste a URL..."
                style={{
                  flex: 1, background: 'transparent', border: '1px solid var(--color-border)',
                  color: 'var(--color-text)', fontSize: 17, fontFamily: 'FS Pixel Sans, monospace',
                  padding: '5px 9px', outline: 'none',
                }}
              />
              <button onClick={() => { setUrlInput(''); setShowUrlInput(false); }} style={{
                background: 'none', border: '1px solid var(--color-border)', color: 'var(--color-text-muted)',
                cursor: 'pointer', fontSize: 16, padding: '4px 10px', fontFamily: 'FS Pixel Sans, monospace',
              }}>✕</button>
            </div>
          )}
          ...
          <div style={{ display: 'flex', gap: 5, position: 'relative' }}>
            {showAttachMenu && (
              <div style={{
                ...
                background: 'var(--color-bg-dark)', border: '1px solid var(--color-border)',
                boxShadow: '0 4px 16px rgba(0,0,0,0.6)', zIndex: 30,
                ...
                {(['📄 File', '🖼 Image', '🔗 URL'] as const).map((label, i) => (
                  <button key={label} onClick={() => {
                    ...
                    cursor: 'pointer', fontSize: 17, padding: '9px 14px', textAlign: 'left',
                    fontFamily: 'FS Pixel Sans, monospace',
                  }}>
                    {label}
                  </button>
            ...
            <button
              ...
              style={{
                ...
                cursor: 'pointer', fontSize: 19, padding: '6px 10px', flexShrink: 0, borderRadius: 2,
              }}
            >📎</button>
            <input
              value={chatInput}
            ...
            <button
              ...
              style={{
                background: 'var(--color-accent)', border: 'none', color: 'var(--color-bg-dark)',
                cursor: (chatInput.trim() || attachments.length > 0) ? 'pointer' : 'default',
                fontSize: 20, fontFamily: 'FS Pixel Sans, monospace',
                padding: '6px 14px', opacity: (chatInput.trim() || attachments.length > 0) ? 1 : 0.4,
                borderRadius: 2,
              }}
            >Send</button>
          </div>
...
export function ActivitySidebar({
  ...
  useEffect(() => {
    const update = () => { nowRef.current = Date.now(); setTick((n) => n + 1); };
    update();
  ...
  useEffect(() => {
    const poll = () => {
      try {
        ...
        setPhaseNames(names ? (JSON.parse(names) as string[]) : []);
        setCurrentPhase(parseInt(localStorage.getItem('pixel-agents-current-phase') ?? '0', 10) || 0);
        setRunMode(localStorage.getItem('pixel-agents-run-mode') ?? '');
        setPhaseGatePending(!!localStorage.getItem('pixel-agents-phase-gate'));
      } catch { /* ignore */ }
    };
    poll();
  ...
  const tokenSummary = useMemo(() => {
    ...
    for (const id of agents) {
      let agentInput = 0, agentOutput = 0;
      ...
      if (agentInput + agentOutput > 0) {
        perAgent.push({ id, name: agentNames[id] ?? `Agent ${id}`, input: agentInput, output: agentOutput });
      }
    }
  ...
  // Project start = earliest history entry timestamp
  ...
  return (
    ...
    >
      ...
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        padding: '10px 14px', borderBottom: '2px solid var(--color-border)', flexShrink: 0,
      }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {onSpawnCeo && (ceoAgentIds?.size ?? 0) === 0 && (
            <button
              ...
              style={{
                background: '#cca70022', border: '1px solid #cca700',
                color: '#cca700', cursor: 'pointer',
          ...
          {agents.length > 0 && (
            <button
              ...
              style={{
                background: 'none', border: '1px solid var(--color-status-permission)',
                color: 'var(--color-status-permission)', cursor: 'pointer',
                fontSize: 16, padding: '2px 8px', lineHeight: 1.4,
                fontFamily: 'FS Pixel Sans, monospace',
              }}
            >End All</button>
      ...
      {/* Tab bar */}
      <div style={{
        display: 'flex', borderBottom: '2px solid var(--color-border)', flexShrink: 0,
      }}>
      ...
      {activeTab === 'agents' && (
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {agents.length === 0 ? (
            <div style={{ padding: '20px 14px', fontSize: 19, color: 'var(--color-text-muted)', textAlign: 'center' }}>
              No agents running.<br />Click + Agent to start.
            </div>
          ) : (
            agents.map((id) => (
              <AgentCard
                ...
                onRelaunch={ceoAgentIds?.has(id) && onSpawnCeo
      ...
      {activeTab === 'summary' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <section>
            <div style={{ fontSize: 13, letterSpacing: '0.12em', color: 'var(--color-text-muted)', marginBottom: 12, textTransform: 'uppercase' }}>
              Active Now
            </div>
            {(() => {
              ...
              if (activeAgents.length === 0) {
                return <div style={{ fontSize: 16, color: 'var(--color-text-muted)' }}>No agents working</div>;
              }
              return (
                  ...
                  {activeAgents.map((id) => {
                    ...
                    return (
                      <div key={id} style={{
                        ...
                        <span className="pixel-pulse" style={{ color: 'var(--color-status-active)', fontSize: 16, flexShrink: 0 }}>▶</span>
                        <div style={{ flex: 1, overflow: 'hidden' }}>
                          <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            #{id} {name}
                          </div>
                          <div style={{ fontSize: 14, color: 'var(--color-text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={currentTool?.status}>
          ...
          <section>
            ...
            {phaseNames.length === 0 ? (
              <div style={{ fontSize: 16, color: 'var(--color-text-muted)' }}>No active project</div>
            ) : (() => {
              const total = phaseNames.length;
              const pct = total > 0 ? Math.min(1, (currentPhase - 1) / total + (currentPhase > 0 ? 0.5 / total : 0)) : 0;
                    ...
                    {phaseNames.map((name, i) => {
                      ...
                      return (
                        <span key={i} style={{
                          ...
                          color: done ? 'var(--color-text-muted)' : active ? 'var(--color-text)' : 'var(--color-border)',
                          borderRadius: 3,
                        }}>
                          {done ? '✓ ' : active ? '● ' : ''}{name}
                        </span>
          ...
          <section>
            ...
            <div style={{ fontSize: 38, fontWeight: 'bold', color: 'var(--color-text)', fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>
            ...
            {projectStart && (
              <div style={{ fontSize: 14, color: 'var(--color-text-muted)', marginTop: 6 }}>
                Started {new Date(projectStart).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </div>
            )}
          </section>
          ...
          <section>
            ...
            {tokenSummary.total === 0 ? (
              <div style={{ fontSize: 16, color: 'var(--color-text-muted)' }}>No turns completed yet</div>
            ) : (
              <>
                {/* Total row */}
                <div style={{ display: 'flex', gap: 16, marginBottom: 18 }}>
                  {[
                    { label: '↑ In', value: tokenSummary.totalInput, color: 'var(--color-text-muted)' },
                    { label: '↓ Out', value: tokenSummary.totalOutput, color: 'var(--color-status-active)' },
                    ...
                    <div key={label} style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 26, fontWeight: 'bold', color, fontVariantNumeric: 'tabular-nums', lineHeight: 1 }}>
                        {formatTokens(value)}
                      </div>
                    </div>
                  ))}
                    ...
                    return (
                      <div key={id}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                          <span style={{ fontSize: 15, color: 'var(--color-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>
                            {name}
                          </span>
