...
export const EditTool = {
  ...
  WALL_EYEDROPPER: 'wall_eyedropper',
  ERASE: 'erase',
  BUCKET: 'bucket',
...
export type ZoneRole = 'work' | 'rest' | 'meeting' | 'outside';
...
export interface RoomZone {
  ...
  /** Height in tiles */
  rows: number;
  /** Hex color string e.g. '#4a9eff' — rendered as translucent fill */
  color: string;
  /** Semantic role — controls agent routing. Falls back to name inference if omitted. */
  role?: ZoneRole;
}
...
export interface OfficeLayout {
  ...
  /** Named room zones overlaid on the canvas */
  zones?: RoomZone[];
...
export interface Character {
  ...
  folderName?: string;
  /** User-assigned display name (overrides "Agent N" label) */
  customName?: string;
  /** User-assigned task description */
  task?: string;
  /** Operating mode set by user */
  mode?: 'default' | 'planner' | 'automation' | 'liberty';
