...
export class OfficeState {
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }): void {
    ...
    for (const ch of this.characters.values()) {
      if (ch.seatId && this.seats.has(ch.seatId)) {
        ...
        if (!seat.assigned) {
          seat.assigned = true;
          // Only snap position when the character is stationary. If they are
          // mid-walk, leave them in place so adding furniture doesn't teleport
          // them. The next time the AI routes them to their seat, they will
          // arrive at the correct (possibly updated) position.
  ...
  /** Walk all listed agents toward the conference table (or map center) for a meeting. */
  walkAgentsToMeetingArea(agentIds: number[]): void {
    // Find CONFERENCE_TABLE furniture in the layout
    const confTable = this.layout.furniture.find((f) => f.type.startsWith('CONFERENCE_TABLE'));

    let centerCol: number;
    let centerRow: number;

    if (confTable) {
      const entry = getCatalogEntry(confTable.type);
      const w = entry?.footprintW ?? 4;
      const h = entry?.footprintH ?? 2;
      centerCol = confTable.col + Math.floor(w / 2);
      centerRow = confTable.row + Math.floor(h / 2);
    } else {
      centerCol = Math.floor(this.layout.cols / 2);
      centerRow = Math.floor(this.layout.rows / 2);
    }

    // Pre-occupy tiles of agents not in this meeting
    const occupied = new Set<string>();
    for (const [id, ch] of this.characters) {
      if (!agentIds.includes(id) && !ch.isSubagent) {
        occupied.add(`${ch.tileCol},${ch.tileRow}`);
      }
    }

    // Sort walkable tiles by Manhattan distance to the meeting center
    const nearby = [...this.walkableTiles].sort((a, b) => {
      const da = Math.abs(a.col - centerCol) + Math.abs(a.row - centerRow);
      const db = Math.abs(b.col - centerCol) + Math.abs(b.row - centerRow);
      return da - db;
    });

    // Assign each agent a unique tile and walk them there
    let tileIdx = 0;
    for (const id of agentIds) {
      while (tileIdx < nearby.length) {
        const tile = nearby[tileIdx++];
        const key = `${tile.col},${tile.row}`;
        if (!occupied.has(key)) {
          occupied.add(key);
          this.walkToTile(id, tile.col, tile.row);
          break;
        }
      }
    }
  ...
  /** Walk an agent to a random tile inside zones with the given role. */
  walkToZone(agentId: number, role: string): boolean {
    const zones = (this.layout.zones ?? []).filter((z) => {
      if (z.role === role) return true;
      const n = z.name.toLowerCase();
      if (role === 'work') return n.includes('work');
      if (role === 'meeting') return n.includes('meet') || n.includes('conf');
      if (role === 'rest') return n.includes('rest') || n.includes('relax') || n.includes('break');
      return false;
    });
    if (zones.length === 0) return false;
    const tiles = this.walkableTiles.filter((t) =>
      zones.some((z) => t.col >= z.col && t.col < z.col + z.cols && t.row >= z.row && t.row < z.row + z.rows),
    );
    if (tiles.length === 0) return false;
    const tile = tiles[Math.floor(Math.random() * tiles.length)];
    return this.walkToTile(agentId, tile.col, tile.row);
  }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      this.withOwnSeatUnblocked(ch, () =>
        updateCharacter(ch, dt, this.walkableTiles, this.seats, this.tileMap, this.blockedTiles, this.layout.zones),
      );
